...
const RECURRING_TASK_SKIPS_KEY = "habit-tracker-recurring-task-skips";

interface RecurringTaskSkip {
  date: string;
  identity: string;
}

function getRecurringTaskIdentity(
  task: Pick<PlannedTask, "habitId" | "title" | "description">,
): string {
  const habitPart = task.habitId ?? "standalone";
  const titlePart = task.title.trim().toLowerCase();
  const descriptionPart = (task.description ?? "").trim().toLowerCase();
  return `${habitPart}::${titlePart}::${descriptionPart}`;
}
...
export const storage = {
  ...
  archiveCurrentYear: (): void => {
    ...
    // Save the year data
    localStorage.setItem(
      `habit-tracker-year-${currentYear}`,
      JSON.stringify(yearData),
    );
  ...
  getRecurringTaskSkips: (): RecurringTaskSkip[] => {
    if (typeof window === "undefined") return [];
    const data = localStorage.getItem(RECURRING_TASK_SKIPS_KEY);
    return data ? JSON.parse(data) : [];
  },

  saveRecurringTaskSkips: (skips: RecurringTaskSkip[]): void => {
    if (typeof window === "undefined") return;
    localStorage.setItem(RECURRING_TASK_SKIPS_KEY, JSON.stringify(skips));
  },
  ...
  getPlannedTasksForDate: (date: string): PlannedTask[] => {
    ...
    const prevDayTasks = tasks.filter(
      (t) => t.date === prevDateStr && t.recurring,
    );
    ...
    prevDayTasks.forEach((prevTask) => {
      const recurringIdentity = getRecurringTaskIdentity(prevTask);
      const isSkippedForDate = recurringTaskSkips.some(
        (skip) => skip.date === date && skip.identity === recurringIdentity,
      );
      if (isSkippedForDate) {
        return;
      }
      ...
      const existsForCurrentDay = currentDayTasks.some(
        (t) =>
          t.title === prevTask.title &&
          t.habitId === prevTask.habitId &&
          t.recurring,
      );
  ...
  getPlannedTasksForHabit: (habitId: string, date: string): PlannedTask[] => {
    const tasks = storage.getPlannedTasks();
    return tasks
      .filter((t) => t.habitId === habitId && t.date === date)
      .sort((a, b) => a.order - b.order);
  },
  ...
  addPlannedTask: (task: PlannedTask): void => {
    ...
    if (task.recurring) {
      ...
      const recurringTaskSkips = storage.getRecurringTaskSkips();
      const filteredSkips = recurringTaskSkips.filter(
        (skip) =>
          !(skip.date === task.date && skip.identity === recurringIdentity),
      );
      if (filteredSkips.length !== recurringTaskSkips.length) {
        storage.saveRecurringTaskSkips(filteredSkips);
      }
    }
  },
  ...
  recalculateHabitCompletion: (habitId: string, date: string): void => {
    ...
    if (habitTasks.length > 0) {
      ...
      const totalTaskWeight = habitTasks.reduce(
        (sum, t) => sum + PRIORITY_VALUES[t.priority],
        0,
      );
      ...
      completionPercentage =
        totalTaskWeight > 0 ? (completedWeight / totalTaskWeight) * 100 : 0;
    }

    const allTasksCompleted =
      habitTasks.length > 0 && habitTasks.every((t) => t.completed);
